...
from src.variables import MAPPER_PATH
from src.configuration import ConfigLogger, SqlTemplate, RowBound, Page
from src.configuration.datasource.mybatis.mapper import PyMapper
from src.configuration.datasource.mybatis.mybatis_exception import PyMybatisException

CONSOLE_LOGGER = ConfigLogger.console_log()


# mapper扫描
class MapperScanner(object):
    def mapper_xml_scan(self, mapper_xml_dir: str):
        mybatis_mapper_dict = MapperDict()
...
class MapperDict(object):
    ...
    def statement(self, sql_id: str, namespace: str = None, **kwargs):
        ...
        CONSOLE_LOGGER.debug(
            "[NAMESPACE:({}) - [SQL_ID:({})] [SQL:({})] - [ARGS:({})]".format(namespace, sql_id, sql, kwargs))
        # print("[NAMESPACE:({}) - [SQL_ID:({})] [SQL:({})] - [ARGS:({})]".format(namespace, sql_id, sql, kwargs))
        return sql
...
class SqlMapper(object):
    ...
    def __init__(self):
        self.mapper_dict = MapperScanner().mapper_xml_scan(mapper_xml_dir=MAPPER_PATH)
        self.sql_template = SqlTemplate()
        if self.sql_template:
            self.datasource = self.sql_template.datasource
        else:
            raise PyMybatisException("sql_template can't init")
